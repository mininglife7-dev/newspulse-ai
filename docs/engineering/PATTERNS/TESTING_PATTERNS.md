...
describe('Workspace Queries Integration', () => {
  ...
  describe('selectById', () => {
    it('should retrieve workspace by ID', async () => {
      ...
      const retrieved = await queries.selectById(supabase, testUserId, created.id)
    ...
    it('should not retrieve workspace from another user', async () => {
      ...
      const retrieved = await queries.selectById(supabase, 'user-b', created.id)
...
describe('Assessment State Transitions', () => {
  it('should allow draft → in_progress transition', async () => {
    ...
    const result = await updateAssessmentStatus('user-1', assessment.id, 'in_progress')
