...
**Last updated:** 2026-07-17 17:25 UTC (RISK-009 added — email confirmation undeliverable; journey mission evidence)
...
| RISK-008 | Production data residency outside the EU — Supabase project was in Tokyo for an EU-compliance product                                                   | —                        | —        | **Closed** | Founder  | ✅ Closed 2026-07-17 — EU end-to-end: DB in Frankfurt (run `29586277262`) + app runtime confirmed (`supabase_host: cwbcvjiklrrkpmybefdp.supabase.co`, run `29596903172`) |
| RISK-009 | Email verification undeliverable in production — "Confirm email" ON + Supabase built-in SMTP (team-only delivery): no real customer can complete signup | Certain (current config) | Critical | **High**   | Founder  | Open — decide: disable Confirm email (fastest) or configure custom SMTP; found by journey run `29599513287`                                                              |
...
### RISK-008 — Production data residency is Tokyo, not EU — ✅ CLOSED 2026-07-17

- **Closure evidence:** DB deployed + verified in Frankfurt (run
  `29586277262`); production app runtime at
  `newspulse-ai-eight.vercel.app` reports
  `supabase_host: cwbcvjiklrrkpmybefdp.supabase.co` with `db: ok`
  (probe run `29596903172`). Full record:
  [EU migration record](../deployments/2026-07-17-EU-MIGRATION-DEPLOY.md).
- Tokyo decommission + password rotation remain Founder-discretion items.
