# Backup & Restore Runbook — NewsPulse AI
...
Each run produces an AES-256-encrypted `newspulse-<timestamp>.dump.gpg` stored as a **private** Actions artifact, retained 90 days.
...
**Rehearsing the restore is the point of R-06 — an untested backup is not a backup.** Do this into a *scratch* Supabase project, never production.

```bash
# 1. Download the artifact from the Actions run, then unzip it. You now have:
#    newspulse-<timestamp>.dump.gpg
