...
| Search timestamp                      | `/history` table, `/history/[id]` header         | One formatter (`formatAbsoluteDate`) via the `LocalDateTime` client component, always in the visitor's timezone | **FIXED** — detail page previously formatted in the _server's_ timezone; the same search could show two different times         |
