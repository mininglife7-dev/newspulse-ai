...
export class CommitMessageValidator {
  ...
  private readonly typePattern = new RegExp(
    `^(${this.validTypes.join('|')})(?:\\(([^)]+)\\))?:\\s(.+)$`,
    'm'
  );
  ...
  validate(message: string): CommitValidationResult {
    ...
    if (!match) {
      ...
      if (typeMatch) {
        ...
        errors.push(
          `Invalid commit type "${invalidType}". Must be one of: ${this.validTypes.join(', ')}`
        );
      } else {
        errors.push(
    ...
    if (!this.validTypes.includes(type)) {
      errors.push(
        `Invalid commit type "${type}". Must be one of: ${this.validTypes.join(', ')}`
      );
    }
    ...
    if (lines.length > 1) {
      for (let i = 2; i < lines.length; i++) {
        ...
        if (line && line.length > 72) {
          errors.push(
            `Line ${i + 1} exceeds 72 characters (${line.length}). Keep body lines concise.`
          );
        }
      }
    }
...
export class BranchNameValidator {
  ...
  private readonly branchPattern = new RegExp(
    `^(${this.validCategories.join('|')})\/([a-z0-9-]+)$`
  );
  ...
  validate(branchName: string): BranchValidationResult {
    ...
    if (branchName === 'main' || branchName === 'master') {
      return {
        ...
        name: branchName,
        errors: [
          'Cannot commit directly to main/master. Create a feature branch instead.',
        ],
      };
    }
  ...
  validate(options: {
    ...
    if (options.baseBranch === 'main' || options.baseBranch === 'master') {
      if (options.isForceUpdate) {
        errors.push(
          'Force-push to main is not allowed. Rebase your branch and retry.'
        );
      }

      if (!options.hasLinearHistory) {
        warnings.push(
          'Merge creates a merge commit. Prefer rebase-and-merge for clean history.'
        );
      }

      if (!options.allChecksPassing) {
        errors.push(
          'Cannot merge: not all status checks are passing. Fix failing checks before merge.'
        );
      }

      if (options.isAutoMerge) {
        errors.push(
          'Auto-merge is not allowed for main branch. Require manual verification.'
        );
      }
    }
...
export class PRValidator {
  ...
  validate(options: {
    ...
    if (!options.title || options.title.length === 0) {
      ...
      warnings.push(
        `PR title exceeds 72 characters (${options.title.length}). Keep titles concise.`
      );
    }
    ...
    if (!Boolean(options.hasLinkedIssue)) {
      warnings.push(
        'Consider adding a linked issue for better traceability (e.g., "Fixes #123")'
      );
    }
