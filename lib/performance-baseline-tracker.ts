...
export class PerformanceBaselineTracker {
  ...
  updateBaseline(metricName: string, thresholdPercentage: number = 20): void {
    const metricsForName = this.recentMetrics.filter((m) => m.name === metricName)
  ...
  detectRegressions(): PerformanceRegression[] {
    ...
    for (const [metricName, baseline] of this.baselines) {
      ...
      const regressionPercentage = ((currentValue - baseline.avgValue) / baseline.avgValue) * 100

      if (regressionPercentage > baseline.thresholdPercentage) {
        const severity = this.calculateSeverity(regressionPercentage, baseline.thresholdPercentage)
        ...
        regressions.push({
          ...
          severity,
          recommendedAction: this.getRecommendedAction(metricName, regressionPercentage),
  ...
  generateReport(): PerformanceReport {
    ...
    return {
      ...
      regressions,
      ok: regressions.filter((r) => r.severity === 'critical' || r.severity === 'high').length === 0,
  ...
  private calculateSeverity(regressionPercentage: number, threshold: number): 'critical' | 'high' | 'medium' | 'low' {
  ...
  private getRecommendedAction(metricName: string, regressionPercentage: number): string {
    const impact = regressionPercentage.toFixed(1)

    if (metricName.includes('latency') || metricName.includes('response-time')) {
