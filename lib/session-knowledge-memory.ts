...
export type KnowledgeDomain = 'architecture' | 'security' | 'performance' | 'reliability' | 'operational' | 'business'
export type DecisionType = 'discovery' | 'decision' | 'action' | 'outcome' | 'metric'
...
export class InMemoryKnowledgeStore implements KnowledgeMemoryStore {
  ...
  async retrieveByDomain(domain: KnowledgeDomain): Promise<KnowledgeEntry[]> {
    return Array.from(this._entries.values()).filter((e) => e.domain === domain)
  }
  ...
  async searchByKeyword(keyword: string): Promise<KnowledgeEntry[]> {
    ...
    return Array.from(this._entries.values()).filter(
      (e) => e.key.toLowerCase().includes(lower) || e.description.toLowerCase().includes(lower)
    )
  }
