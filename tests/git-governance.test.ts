...
describe('git-governance (DNA-GOV-010)', () => {
  describe('CommitMessageValidator', () => {
    ...
    it('rejects message with invalid type', () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('Invalid commit type'))).toBe(true)
    })
...
This is a very long line that exceeds the recommended 72 character limit and should fail validation`
    ...
    it('accepts all valid commit types', () => {
      const types = ['feat', 'fix', 'docs', 'style', 'refactor', 'perf', 'test', 'chore', 'ci']
  ...
  describe('BranchNameValidator', () => {
    ...
    it('accepts all valid categories', () => {
      const categories = ['feature', 'fix', 'docs', 'refactor', 'chore', 'test', 'perf']
  ...
  describe('MergeValidator', () => {
    ...
    it('rejects merge when checks failing', () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('not all status checks'))).toBe(true)
    })
    ...
    it('warns about merge commits to main', () => {
      ...
      expect(result.warnings.some((w) => w.includes('merge commit'))).toBe(true)
    })
  ...
  describe('PRValidator', () => {
    ...
    it('warns about long title', () => {
      const result = validator.validate({
        title: 'feat: add very long title that exceeds the recommended 72 character limit and should warn',
        commitMessages: ['feat: add feature'],
      })
      expect(result.valid).toBe(true)
      expect(result.warnings.some((w) => w.includes('72 characters'))).toBe(true)
    ...
    it('validates multiple commits', () => {
      const result = validator.validate({
        ...
        commitMessages: ['feat(auth): add google oauth', 'test(auth): add oauth tests'],
      })
      expect(result.valid).toBe(true)
      expect(result.checks.messageFollowsConvention).toBe(true)
    })
